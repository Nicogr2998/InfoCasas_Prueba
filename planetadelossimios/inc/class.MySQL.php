...
switch ($_SERVER['SERVER_NAME']) {
	case 'trabajos.impactus.com.uy':
	case 'www.trabajos.impactus.com.uy':
		define('_dbname_','pulga_fox_epdls');
		define('_dbuser_','pulga_db');
		define('_dbpass_','itunes');
		break;
	case 'simios.dev':
		define('_dbname_','fox_epdls');
		define('_dbuser_','root');
		define('_dbpass_','');
		break;

	default:
		define('_dbname_','heroku_b55bf73938752be');
		define('_dbuser_','b9e1808cadc2ea');
		define('_dbpass_','663c8a2b');
		define('_dbhost_', 'us-cdbr-east-04.cleardb.com');
		break;
}
...
class MySQL {
	...
	function __construct($database=_dbname_, $username=_dbuser_, $password=_dbpass_, $hostname=_dbhost_, $port=3306){
	...
	private function Connect($persistant = false){
		...
		$this->ExecuteSQL("SET NAMES 'utf8'"); // con esto funcionan los tildes y esas manos
		return true;
	}
	...
	function Insert($vars, $table, $exclude = ''){
		...
		foreach($vars as $key=>$value){
			...
			//$value = $this->limpiarString($value);
			//$query .= '`' . $key . '` = "' . $value . '", ';
			$query .= "`{$key}` = '{$value}', ";
		}
